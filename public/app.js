...
const pathAudioCache=new Map();let activeBattleSfx=0,lastImpactSoundAt=0;
function reserveBattleSfx(priority=false){if(!state.battle)return true;const limit=state.battle.fighters?.length>=8?7:10,maxChannels=limit+(priority?4:0);if(activeBattleSfx>=maxChannels)return false;activeBattleSfx++;return true}
function trackBattleSfx(audio,reserved){if(!reserved||!state.battle)return;let released=false;const release=()=>{if(released)return;released=true;activeBattleSfx=Math.max(0,activeBattleSfx-1)};audio.addEventListener("ended",release,{once:true});audio.addEventListener("error",release,{once:true});setTimeout(release,3200)}
function playSound(name, volume=.4, rate=1,priority=false) {
  if (!state.sound || !sounds[name]) return null;
  const reserved=reserveBattleSfx(priority);if(!reserved)return null;const sound=sounds[name].cloneNode();sound.volume=volume;sound.playbackRate=rate;trackBattleSfx(sound,reserved);sound.play().catch(()=>{});return sound;
}
function playPathSound(path,volume=.4,rate=1,delay=0,priority=false){if(!state.sound)return null;const play=()=>{const reserved=reserveBattleSfx(priority);if(!reserved)return null;if(!pathAudioCache.has(path)){const base=new Audio(path);base.preload="auto";pathAudioCache.set(path,base)}const audio=pathAudioCache.get(path).cloneNode();audio.volume=volume;audio.playbackRate=rate;trackBattleSfx(audio,reserved);audio.play().catch(()=>{});return audio};if(delay){setTimeout(play,delay);return null}return play()}
...
function playUltimateSound(event){const file=ULTIMATE_AUDIO[event.universeId]||"explosionCrunch_003.ogg";playPathSound(`assets/sounds/abilities/${file}`,.7,.9+hashText(event.ultimateId||event.universeId)%24/100,0,true);setTimeout(()=>playPathSound(`assets/sounds/abilities/${ultimateVisualAudio(event)}`,.52,.86+hashText(event.name)%22/100,0,true),110);const accent=event.universeId==="naruto"?"sharingan":event.universeId==="jojo"?"time-stop":event.type==="domain"?"domain":event.type==="atomic"?"beam":"ultimate";setTimeout(()=>playSound(accent,.48,1,true),180)}
...
let arenaBackdropCache=null,arenaDangerGradient=null;
...
function crowdedBattle(){return (state.battle?.fighters?.length||0)>=7}
function battleRenderInterval(){return 13}
function trackBattleFps(now){const b=state.battle;if(!b)return;b.fpsFrames=(b.fpsFrames||0)+1;const elapsed=now-(b.fpsWindowStart||now);if(elapsed<500)return;const fps=Math.round(b.fpsFrames*1000/elapsed),badge=$("#battleFps");badge.textContent=`${fps} FPS`;badge.classList.toggle("low",fps<50);b.fpsFrames=0;b.fpsWindowStart=now}
function startBattle(seed) {
  ...
  state.battle={fighters,projectiles:[],effects:[],blood:[],texts:[],delayed:[],ultimatePulses:[],ultimateEvent:null,ultimateEventId:0,actionEvents:[],actionEventId:0,feed:[],feedKey:"",lastFeedSyncKey:"",time:0,serverTime:0,last:performance.now(),lastViewRender:0,lastArenaRender:0,fpsFrames:0,fpsWindowStart:performance.now(),accumulator:0,over:false,resultShown:false,lastSync:0,lastHudRender:0,powerAiming:false,aimX:W/2,aimY:H/2,zone:{left:8,right:W-8,top:8,bottom:H-8,start:50,duration:55,maxX:205,maxY:120,announced:false},raf:null};$("#battleFps").textContent="60 FPS";$("#battleFps").classList.remove("low");
...
function battleLoop(now) {
  ...
  while(!b.over&&b.accumulator>=1/60){updateBattle(1/60);b.time+=1/60;b.accumulator-=1/60;}if(now-(b.lastArenaRender||0)>=battleRenderInterval(true)){b.lastArenaRender=now;trackBattleFps(now);drawBattle()}syncBattle(now);b.raf=requestAnimationFrame(battleLoop);
}
function battleViewLoop(now) {const b=state.battle;if(!b)return;const frameInterval=battleRenderInterval(false);if(now-(b.lastViewRender||0)<frameInterval){b.raf=requestAnimationFrame(battleViewLoop);return}const dt=Math.min(.1,(now-b.last)/1000||0);b.last=now;b.lastViewRender=now;if(!b.over){b.time+=dt;const correction=(b.serverTime??b.time)-b.time;b.time+=Math.max(-.08,Math.min(.08,correction))*Math.min(1,dt*8)}for(const fighter of b.fighters){if(!Number.isFinite(fighter.netX))continue;const blend=Math.min(1,dt*14);fighter.x+=(fighter.netX-fighter.x)*blend;fighter.y+=(fighter.netY-fighter.y)*blend}for(const shot of b.projectiles){shot.x+=shot.vx*dt;shot.y+=shot.vy*dt}b.effects.forEach(effect=>effect.life=Math.max(0,effect.life-dt));b.blood?.forEach(drop=>drop.life=Math.max(0,drop.life-dt));b.texts.forEach(text=>{text.life=Math.max(0,text.life-dt);text.y-=25*dt});trackBattleFps(now);drawBattle();b.raf=requestAnimationFrame(battleViewLoop);}
function updateBattle(dt) {
  ...
  b.effects.forEach(e=>e.life-=dt);b.effects=b.effects.filter(e=>e.life>0).slice(crowdedBattle()?-24:-64);b.blood.forEach(drop=>drop.life-=dt);b.blood=b.blood.filter(drop=>drop.life>0).slice(crowdedBattle()?-36:-96);b.texts.forEach(t=>{t.y-=25*dt;t.life-=dt});b.texts=b.texts.filter(t=>t.life>0).slice(crowdedBattle()?-10:-24);
...
function updateZone() {
  ...
  const label=progress<=0?`ЗОНА ЧЕРЕЗ ${Math.max(0,Math.ceil(50-b.time))}с`:progress<1?`ЗОНА: ${Math.round(progress*100)}%`:"ЗОНА: МАКСИМУМ";if(label!==b.zoneLabel){b.zoneLabel=label;$("#zoneStatus").textContent=label}
}
...
function requestBasicAttack(x,y){const fighter=localFighter(),b=state.battle;if(!fighter?.alive||!b||b.over)return;const recovery=basicAttackProfile(fighter).recovery;if(b.time-fighter.lastBasic<recovery)return;if(isHost())performManualAttack(fighter,x,y);else{fighter.lastBasic=b.time;send({type:"attack_request",x,y})}}
...
function syncBattle(now){
  const b=state.battle,syncInterval=crowdedBattle()?180:150;if(!isHost()||now-b.lastSync<syncInterval)return;b.lastSync=now;const feedKey=b.feed.map(item=>`${item.time}:${item.html}`).join("|"),feed=feedKey!==b.lastFeedSyncKey?b.feed:undefined;b.lastFeedSyncKey=feedKey;
  send({type:"battle_state",battle:{time:b.time,over:b.over,randomSeed:state.randomSeed,zone:b.zone,fighters:b.fighters.map(compactFighter),projectiles:b.projectiles.slice(-48),effects:b.effects.slice(crowdedBattle()?-16:-32),texts:b.texts.slice(crowdedBattle()?-8:-12),delayed:b.delayed.slice(-24),ultimatePulses:b.ultimatePulses,ultimateEvent:b.ultimateEvent,ultimateEventId:b.ultimateEventId,actionEvents:b.actionEvents.slice(-8),actionEventId:b.actionEventId,feed}});
}
...
function drawBattle(){
  ...
  if(!arenaDangerGradient){arenaDangerGradient=ctx.createLinearGradient(0,0,W,H);arenaDangerGradient.addColorStop(0,"rgba(255,35,55,.2)");arenaDangerGradient.addColorStop(.5,"rgba(68,12,22,.08)");arenaDangerGradient.addColorStop(1,"rgba(167,42,255,.18)")}const z=b.zone;ctx.fillStyle=arenaDangerGradient;ctx.fillRect(0,0,z.left,H);ctx.fillRect(z.right,0,W-z.right,H);ctx.fillRect(z.left,0,z.right-z.left,z.top);ctx.fillRect(z.left,z.bottom,z.right-z.left,H-z.bottom);ctx.strokeStyle=z.announced?"#ff4b58":"rgba(167,208,255,.42)";ctx.shadowColor=ctx.strokeStyle;ctx.shadowBlur=crowdedBattle()?0:z.announced?18:7;ctx.lineWidth=5;ctx.strokeRect(z.left,z.top,z.right-z.left,z.bottom-z.top);ctx.shadowBlur=0;
  drawBlood();for(const f of b.fighters)if(f.alive&&f.cagedUntil>b.time)drawCage(f);for(const shot of b.projectiles)drawProjectile(shot);for(const f of b.fighters)if(f.alive)drawFighter(f);drawPowerPreview();for(const effect of b.effects)drawEffect(effect);for(const text of b.texts){ctx.globalAlpha=Math.max(0,text.life/.8);ctx.fillStyle=text.color;ctx.font="700 15px Rubik";ctx.textAlign="center";ctx.shadowColor="#000";ctx.shadowBlur=crowdedBattle()?0:5;ctx.fillText(text.text,text.x,text.y);ctx.globalAlpha=1;ctx.shadowBlur=0}updateHud();updateDeathNoteWarning();
}
...
function drawBlood(){
  const drops=state.battle?.blood||[];ctx.save();ctx.fillStyle="#8d0715";ctx.shadowColor="#330006";ctx.shadowBlur=crowdedBattle()?0:3;
...
function drawCage(f){
  const time=state.battle.time,color=f.cageColor||"#985dff",radius=f.radius+31,lowFx=crowdedBattle(),bars=lowFx?6:10;ctx.save();ctx.translate(f.x,f.y);ctx.strokeStyle=color;ctx.fillStyle=color;ctx.shadowColor=color;ctx.shadowBlur=lowFx?6:18;ctx.globalAlpha=.72;ctx.lineWidth=4;ctx.beginPath();ctx.ellipse(0,-f.radius*.1,radius,radius*.36,0,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.ellipse(0,f.radius*.45,radius,radius*.36,0,0,Math.PI*2);ctx.stroke();for(let bar=0;bar<bars;bar++){const angle=bar*Math.PI*2/bars+time*.25,x=Math.cos(angle)*radius,y=Math.sin(angle)*radius*.35;ctx.globalAlpha=.35+.55*(Math.sin(angle)+1)/2;ctx.lineWidth=3+2*(Math.sin(angle)+1)/2;ctx.beginPath();ctx.moveTo(x,y-f.radius*.75);ctx.lineTo(x,y+f.radius*.7);ctx.stroke()}ctx.rotate(-time*.8);ctx.setLineDash([6,8]);ctx.beginPath();ctx.arc(0,0,radius+9,0,Math.PI*2);ctx.stroke();ctx.restore();
}
...
function drawFighter(f){
  const look=cleanAppearance(f.appearance),time=state.battle.time,r=f.radius,lowFx=crowdedBattle();ctx.save();ctx.translate(f.x,f.y);if(look.aura!=="none"){ctx.save();ctx.globalAlpha=lowFx?.34:.5;ctx.strokeStyle=look.aura==="flame"?"#ff6538":look.aura==="electric"?"#61eaff":"#9b58ff";ctx.shadowColor=ctx.strokeStyle;ctx.shadowBlur=lowFx?4:18;ctx.lineWidth=look.aura==="electric"?3:7;ctx.setLineDash(look.aura==="electric"?[4,7]:[]);ctx.rotate(time*(look.aura==="shadow"?-.8:1.4));ctx.beginPath();ctx.arc(0,0,r+9+Math.sin(time*6)*3,0,Math.PI*2);ctx.stroke();ctx.restore()}drawWeapon(f);ctx.shadowColor=f.universe.color;ctx.shadowBlur=lowFx?(f.attackFlash>0?10:3):(f.attackFlash>0?28:12);ctx.fillStyle=f.hitFlash>0?"#fff":f.color;ctx.strokeStyle="#050507";ctx.lineWidth=5;ctx.beginPath();ctx.arc(0,0,r,0,Math.PI*2);ctx.fill();ctx.stroke();if(look.pattern==="split"){ctx.save();ctx.beginPath();ctx.arc(0,0,r-3,-1.1,1.1);ctx.lineTo(0,0);ctx.closePath();ctx.fillStyle="rgba(255,255,255,.28)";ctx.fill();ctx.restore()}else if(look.pattern==="ring"){ctx.strokeStyle="rgba(255,255,255,.62)";ctx.lineWidth=3;ctx.beginPath();ctx.arc(0,0,r*.68,0,Math.PI*2);ctx.stroke()}else if(look.pattern==="core"){ctx.fillStyle="#fff";ctx.shadowColor="#fff";ctx.shadowBlur=lowFx?3:12;ctx.beginPath();ctx.arc(0,r*.38,r*.15,0,Math.PI*2);ctx.fill()}if(f.shield>0){ctx.strokeStyle="#7de8ff";ctx.lineWidth=4;ctx.globalAlpha=.7;ctx.beginPath();ctx.arc(0,0,r+7,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1}
...
function drawProjectile(shot){
  const owner=state.battle.fighters.find(f=>f.id===shot.ownerId),power=owner?.power,image=power?imageFor(power.icon):null,color=shot.color||owner?.universe.color||"#fff",type=shot.powerType||power?.type||"projectile",trail=type==="beam"?150:type==="mark"?95:type==="bomb"?42:72,lowFx=crowdedBattle();
  ctx.save();ctx.translate(shot.x,shot.y);ctx.rotate(shot.angle);ctx.lineCap="round";ctx.shadowColor=color;ctx.shadowBlur=lowFx?2:26;
  if(lowFx){ctx.strokeStyle=color;ctx.globalAlpha=.82}else{const gradient=ctx.createLinearGradient(-trail,0,14,0);gradient.addColorStop(0,"transparent");gradient.addColorStop(.7,color);gradient.addColorStop(1,"#fff");ctx.strokeStyle=gradient}
...
function drawEffect(e){
  const progress=1-e.life/e.maxLife,base=e.radius||20,alpha=Math.max(0,e.life/e.maxLife),lowFx=crowdedBattle();ctx.save();ctx.globalAlpha=alpha;ctx.strokeStyle=e.color;ctx.fillStyle=e.color;ctx.shadowColor=e.color;ctx.shadowBlur=lowFx?(e.type==="ultimate"?10:3):(e.type==="ultimate"?24:14);
