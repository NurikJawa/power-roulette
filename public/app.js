...
function performManualAttack(fighter,aimX,aimY){
  ...
  const angle=Math.atan2(aimY-fighter.y,aimX-fighter.x),style=attackStyle(fighter),reach=fighter.radius+(["magic","psychic","ki"].includes(style)?105:style==="drill"?92:82),event={id:++b.actionEventId,kind:"basic",ownerId:fighter.id,style};fighter.attackAngle=angle;b.actionEvents.push(event);b.actionEvents=b.actionEvents.slice(-16);lastActionEventId=event.id;playBasicSound(style,event.id);
  ...
  if(!victims.length){addText(aimX,aimY,"МИМО",fighter.color);return}const base=(20+random()*10)*fighter.damageMult*(b.time>120?1.45:b.time>90?1.2:1);for(const target of victims)dealDamage(target,base,fighter,{name:{blade:"удар клинком",drill:"удар буром",chainsaw:"разрез бензопилой",magic:"магический удар",psychic:"психоимпульс",ki:"удар ки",alchemy:"трансмутация",notebook:"удар приговором",kagune:"удар кагуне"}[style]||"обычный удар",armorPen:0,knockback:105},random()<fighter.crit);
}
...
function abilityVisual(power){const value=`${power.id} ${power.name}`.toLowerCase();if(/drill|spiral|бур|спирал/.test(value))return "drill";if(/chainsaw|бензопил/.test(value))return "chainsaw";if(/kagune|rinkaku|ukaku|koukaku|bikaku|кагун/.test(value))return "kagune";if(/notebook|shinigami|тетрад|приговор/.test(value))return "notebook";if(/alchemy|transmut|алхим|трансмута/.test(value))return "alchemy";if(/sun|fire|flame|magma|огн|солн/.test(value)&&/breath|slash|sword|blade|katana|клин|меч|дыхани/.test(value))return "fireBlade";if(/water|вода|водн/.test(value)&&/breath|slash|sword|blade|katana|клин|меч|дыхани/.test(value))return "waterBlade";if(/thunder|lightning|chidori|electric|молн|гром/.test(value))return "lightning";if(/slash|sword|blade|katana|zangetsu|breath|claw|dagger|клин|меч|дыхани/.test(value))return "blade";if(/fire|flame|magma|atomic|explosion|bomb|огн|взрыв/.test(value))return "explosion";if(/shadow|dark|curse|тен|тьм/.test(value))return "shadow";if(/psychic|telekin|mind|domain|псих|телекин/.test(value))return "psychic";return power.type}
...
function drawWeapon(f){
  const weapon=f.universe.battleStyle?.weapon||"fist",angle=Number.isFinite(f.attackAngle)?f.attackAngle:Math.atan2(f.vy,f.vx),swing=f.attackFlash*Math.sin((1-f.attackFlash)*Math.PI)*.85,r=f.radius;ctx.save();ctx.rotate(angle+swing);ctx.translate(r*.66,0);ctx.strokeStyle="#fff";ctx.fillStyle=f.universe.color;ctx.shadowColor=f.universe.color;ctx.shadowBlur=13;ctx.lineCap="round";ctx.lineJoin="round";
  if(["katana","nichirin","cursedBlade","slimeSword"].includes(weapon)){ctx.lineWidth=weapon==="nichirin"?5:7;ctx.beginPath();ctx.moveTo(-r*.1,0);ctx.lineTo(r*1.55,0);ctx.stroke();ctx.strokeStyle=weapon==="nichirin"?"#ff713f":"#fff";ctx.lineWidth=2;ctx.stroke();ctx.fillStyle="#17171c";ctx.fillRect(-r*.28,-5,r*.36,10)}
