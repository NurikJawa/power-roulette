...
function battleRenderInterval(host=false){if(runtimeLiteFx)return host?42:50;if(crowdedBattle())return host?33:42;return host?20:33}
function startBattle(seed) {
  ...
  state.battle={fighters,projectiles:[],effects:[],blood:[],texts:[],delayed:[],ultimatePulses:[],ultimateEvent:null,ultimateEventId:0,actionEvents:[],actionEventId:0,feed:[],feedKey:"",lastFeedSyncKey:"",time:0,serverTime:0,last:performance.now(),lastViewRender:0,lastArenaRender:0,accumulator:0,over:false,resultShown:false,lastSync:0,lastHudRender:0,powerAiming:false,aimX:W/2,aimY:H/2,zone:{left:8,right:W-8,top:8,bottom:H-8,start:50,duration:55,maxX:205,maxY:120,announced:false},raf:null};
...
function battleLoop(now) {
  ...
  while(!b.over&&b.accumulator>=1/60){updateBattle(1/60);b.time+=1/60;b.accumulator-=1/60;}if(now-(b.lastArenaRender||0)>=battleRenderInterval(true)){b.lastArenaRender=now;drawBattle()}syncBattle(now);b.raf=requestAnimationFrame(battleLoop);
}
...
function updateBattle(dt) {
  ...
  b.effects.forEach(e=>e.life-=dt);b.effects=b.effects.filter(e=>e.life>0).slice(crowdedBattle()?-36:-64);b.blood.forEach(drop=>drop.life-=dt);b.blood=b.blood.filter(drop=>drop.life>0).slice(crowdedBattle()?-48:-96);b.texts.forEach(t=>{t.y-=25*dt;t.life-=dt});b.texts=b.texts.filter(t=>t.life>0).slice(crowdedBattle()?-12:-24);
...
function drawProjectile(shot){
  ...
  ctx.save();ctx.translate(shot.x,shot.y);ctx.rotate(shot.angle);ctx.lineCap="round";ctx.shadowColor=color;ctx.shadowBlur=crowdedBattle()?8:26;
