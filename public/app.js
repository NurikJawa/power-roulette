...
function currentProfile() {
  const name=$("#onlineName").value.trim()||"Игрок",color=$("#onlineColor").value;
  localStorage.setItem("powerRouletteName",name); localStorage.setItem("powerRouletteColor",color); return {name,color};
}
...
function handleMessage(message) {
  if (message.type==="hello") {
    ...
    const fighter=state.battle?.fighters.find(item=>item.clientId===message.senderId);
    if(fighter)activatePowerFor(fighter,message.senderId);
  }
...
function renderLobby() {
  ...
  $("#onlinePlayers").innerHTML=room().players.map((player,index)=>`<div class="online-player ${player.id===state.multiplayer.clientId?"me":""}"><div class="player-orb" style="background:${player.color}"><i></i><i></i></div><strong>${escapeHtml(player.name)}</strong><small>${index===0?"ВЕДУЩИЙ":`ИГРОК ${index+1}`}</small></div>`).join("");
...
function basePlayer(player,index) { return { id:index,clientId:player.id,name:player.name,color:player.color||PLAYER_COLORS[index],universe:null,race:null,power:null,attrs:{},damageDone:0,kills:0 }; }
...
function resultForPhase(player) {
  const id=currentPhase().id; if (id==="universe") return player.universe; if(id==="race")return player.race;if(id==="power")return player.power;return player.attrs[id];
}
function updatePhaseUi() {
  ...
  $("#spinHint").textContent=phase.type==="race"?"В колесе только расы выпавшей вселенной":phase.type==="power"?"Пять уникальных сил именно этой вселенной":phase.type==="universal"?"Общая характеристика влияет на формулы боя":"21 проработанная вселенная — решает случай";
}
...
function assignResult(player,item) {
  const id=currentPhase().id; if(id==="universe")player.universe=item;else if(id==="race")player.race=item;else if(id==="power")player.power=item;else player.attrs[id]=item;
}
...
function showResult(item) {
  ...
  const stats=[]; if(item.damage)stats.push(`УРОН ${item.damage}`);if(item.cooldown)stats.push(`КД ${item.cooldown}с`);if(item.range)stats.push(`ДАЛЬНОСТЬ ${item.range}`);if(item.hp)stats.push(`HP ×${item.hp}`);if(item.armor)stats.push(`БРОНЯ ${Math.round(item.armor*100)}%`);if(item.label)stats.push(item.label);
...
function showBuild() {
  ...
  const rows=[{label:"ВСЕЛЕННАЯ",item:player.universe},{label:"РАСА",item:player.race},{label:"УНИКАЛЬНАЯ СИЛА",item:player.power},...UNIVERSAL_ROULETTES.map(r=>({label:r.name,item:player.attrs[r.id]}))].filter(row=>row.item);
...
function startBattle(seed) {
  ...
  const fighters=state.players.map((player,index)=>{
    ...
    return {...player,x:W/2+Math.cos(angle)*(W/2-padding),y:H/2+Math.sin(angle)*(H/2-padding),vx:Math.cos(direction)*moveSpeed,vy:Math.sin(direction)*moveSpeed,moveSpeed,radius:size,hp:maxHp,maxHp,armor:Math.min(.28,r.armor+stat(player,"durability","armor",0)),damageMult:r.damage*stat(player,"strength","damage"),cooldown:p.cooldown*stat(player,"iq","cooldown"),crit:Math.max(0,Math.min(.42,(p.crit||0)+addStat(player,"crit"))),dodge:Math.max(0,Math.min(.3,(p.dodge||0)+addStat(player,"dodge"))),alive:true,shield:0,lastPower:-99,lastBasic:-99,stunnedUntil:0,dotUntil:0,nextDot:0,dotDamage:0,dotOwner:null,reviveAvailable:!!player.attrs.luck?.stats?.revive,revives:0,damageDone:0,kills:0,attacks:0,hits:0,dodges:0,blocks:0,hitFlash:0,attackFlash:0,ultimateReadyAt:0,ultimateActiveUntil:0,ultimateDamageMult:1,ultimateSpeedMult:1,ultimateLifesteal:0,ultimateDodge:0,slowedUntil:0,slowFactor:1};
  });
  lastUltimateEventId=0;lastActionEventId=0;
  state.battle={fighters,projectiles:[],effects:[],texts:[],delayed:[],ultimatePulses:[],ultimateEvent:null,ultimateEventId:0,actionEvents:[],actionEventId:0,feed:[],time:0,serverTime:0,last:performance.now(),accumulator:0,over:false,resultShown:false,lastSync:0,lastHudRender:0,zone:{left:8,right:W-8,top:8,bottom:H-8,start:50,duration:55,maxX:205,maxY:120,announced:false},raf:null};
...
function updateBattle(dt) {
  ...
  b.effects.forEach(e=>e.life-=dt);b.effects=b.effects.filter(e=>e.life>0);b.texts.forEach(t=>{t.y-=25*dt;t.life-=dt});b.texts=b.texts.filter(t=>t.life>0);
...
function attackStyle(fighter){return /slash|sword|blade|katana|zangetsu|breath|claw|chainsaw|dagger|клин|меч|дыхани/i.test(`${fighter.power.id} ${fighter.power.name}`)||["bleach","demon-slayer","black-clover","chainsaw","eminence"].includes(fighter.universe.id)?"blade":"strike"}
function playBasicSound(style,id=0){const variant=Math.abs(id)%3;if(style==="blade")playPathSound(`assets/sounds/abilities/rpg-swing${variant?variant+1:""}.ogg`,.48,.92+variant*.08);else playPathSound(`assets/sounds/abilities/impactPunch_medium_${String(variant).padStart(3,"0")}.ogg`,.4,.94+variant*.07)}
...
function performManualAttack(fighter,aimX,aimY){
  ...
  const angle=Math.atan2(aimY-fighter.y,aimX-fighter.x),style=attackStyle(fighter),reach=fighter.radius+82,event={id:++b.actionEventId,kind:"basic",ownerId:fighter.id,style};b.actionEvents.push(event);b.actionEvents=b.actionEvents.slice(-16);lastActionEventId=event.id;playBasicSound(style,event.id);
  b.effects.push({type:"aim",x:aimX,y:aimY,color:fighter.color,life:.48,maxLife:.48,radius:24},{type:style==="blade"?"slash":"punch",x:fighter.x,y:fighter.y,angle,color:fighter.universe.color,life:.36,maxLife:.36,radius:reach});
  ...
  if(!victims.length){addText(aimX,aimY,"МИМО",fighter.color);return}const base=(20+random()*10)*fighter.damageMult*(b.time>120?1.45:b.time>90?1.2:1);for(const target of victims)dealDamage(target,base,fighter,{name:style==="blade"?"удар клинком":"обычный удар",armorPen:0,knockback:105},random()<fighter.crit);
}
...
function requestPower(){
  ...
  if(isHost())activatePowerFor(fighter,fighter.clientId);else send({type:"power_request"});
}
function activatePowerFor(fighter,clientId){
  ...
  const enemies=b.fighters.filter(target=>target.alive&&target!==fighter),target=fighter.power.type==="precision"?[...enemies].sort((a,c)=>a.hp/a.maxHp-c.hp/c.maxHp)[0]:nearestEnemy(fighter,enemies);
  if(!target)return powerFeedback(clientId,"На арене нет цели");const result=tryPower(fighter,target);
  if(result==="range")powerFeedback(clientId,`${fighter.power.name}: цель слишком далеко`);
}
...
function activateUltimate(fighter){
  const b=state.battle,ultimate=fighter?.universe?.ultimate;if(!isHost()||!b||b.over||!fighter?.alive||!ultimate||b.time<fighter.ultimateReadyAt)return;
  ...
  const event={id:++b.ultimateEventId,ownerId:fighter.id,ownerName:fighter.name,universeId:fighter.universe.id,color:fighter.universe.color,icon:ultimate.icon,name:ultimate.name,description:ultimate.description,type:ultimate.type,at:b.time};
  ...
  if(["nova","rumbling","atomic"].includes(ultimate.type)){
    ...
    for(let index=0;index<pulses;index++)b.ultimatePulses.push({at:b.time+.45+index*.72,kind:"wave",ownerId:fighter.id,damage:ultimate.damage,name:ultimate.name,slow:ultimate.slow||0});
  }else if(["buff","berserk","armorBreak"].includes(ultimate.type)){
    fighter.ultimateDamageMult=ultimate.damage||1.3;fighter.ultimateSpeedMult=ultimate.speed||1.18;fighter.ultimateLifesteal=ultimate.heal||0;
    if(ultimate.type==="armorBreak")fighter.shield=Math.max(fighter.shield,70);
  }else if(ultimate.type==="alchemy"){
    fighter.hp=Math.min(fighter.maxHp,fighter.hp+(ultimate.heal||220));fighter.ultimateDamageMult=ultimate.damage||1.25;fighter.shield=Math.max(fighter.shield,110);addText(fighter.x,fighter.y,`+${ultimate.heal||220} HP`,"#d8ff45");
  }
  for(let ring=0;ring<5;ring++)b.effects.push({type:"ultimate",x:fighter.x,y:fighter.y,color:fighter.universe.color,life:.5+ring*.14,maxLife:.5+ring*.14,radius:35+ring*32});
}
function updateUltimates(){
  ...
  for(const pulse of b.ultimatePulses){
    ...
    const owner=b.fighters.find(f=>f.id===pulse.ownerId);if(!owner?.alive)continue;
    if(pulse.kind==="execute"){
...
function showUltimateCinematic(event){
  ...
  panel.className=`ultimate-cinematic ${event.universeId}`;panel.style.setProperty("--ultimate-color",event.color||"#9b65ff");
...
function abilityVisual(power){const value=`${power.id} ${power.name}`.toLowerCase();if(/sun|fire|flame|magma|огн|солн/.test(value)&&/breath|slash|sword|blade|katana|клин|меч|дыхани/.test(value))return "fireBlade";if(/water|вода|водн/.test(value)&&/breath|slash|sword|blade|katana|клин|меч|дыхани/.test(value))return "waterBlade";if(/thunder|lightning|chidori|electric|молн|гром/.test(value))return "lightning";if(/slash|sword|blade|katana|zangetsu|breath|claw|chainsaw|dagger|клин|меч|дыхани/.test(value))return "blade";if(/fire|flame|magma|atomic|explosion|bomb|огн|взрыв/.test(value))return "explosion";if(/shadow|dark|curse|тен|тьм/.test(value))return "shadow";if(/psychic|telekin|mind|domain|псих|телекин/.test(value))return "psychic";return power.type}
function emitActionEvent(attacker,target){
  const b=state.battle,power=attacker.power,event={id:++b.actionEventId,ownerId:attacker.id,powerId:power.id,type:power.type,universeId:attacker.universe.id};b.actionEvents.push(event);b.actionEvents=b.actionEvents.slice(-16);lastActionEventId=event.id;playAbilitySound(power,attacker.universe.id);
...
function tryPower(attacker,target){const b=state.battle,p=attacker.power;if(b.time-attacker.lastPower<attacker.cooldown)return "cooldown";const d=distance(attacker,target),selfCast=["heal","shield"].includes(p.type),bodyReach=attacker.radius+target.radius;if(!selfCast&&["melee","drain","counter","dash","bounce","teleport"].includes(p.type)&&d>p.range+bodyReach)return "range";const ranged=["projectile","beam","bomb","control","mark","precision","dot","area"].includes(p.type);if(ranged&&d>p.range*1.45+bodyReach)return "range";attacker.lastPower=b.time;attacker.attacks++;attacker.attackFlash=1;emitActionEvent(attacker,target);
  ...
  const targets=p.area?b.fighters.filter(f=>f.alive&&f!==attacker&&distance(attacker,f)<p.area):[target];for(const victim of targets)dealDamage(victim,damage,attacker,p,random()<attacker.crit);
  return "ok";
}
...
function renderSkillHud(){
  ...
  hud.classList.remove("hidden");hud.classList.toggle("ready",ready);hud.classList.toggle("disabled",!fighter.alive);hud.style.setProperty("--ability-color",fighter.universe.color);$("#skillIcon").src=power.icon;$("#skillName").textContent=power.name;$("#skillBar").style.width=`${ready?100:Math.max(0,(1-remaining/fighter.cooldown)*100)}%`;$("#skillStatus").textContent=!fighter.alive?"БОЕЦ ВЫБЫЛ":ready?`E — ${power.type==="dash"?"РЫВОК":"АКТИВИРОВАТЬ"}`:`ПЕРЕЗАРЯДКА ${remaining.toFixed(1)}с`;
}
function renderAbilityHud(){
  ...
  const ultimate=fighter.universe.ultimate,remaining=Math.max(0,fighter.ultimateReadyAt-b.time),active=b.time<fighter.ultimateActiveUntil,ready=fighter.alive&&!active&&remaining<=0;
...
const FIGHTER_SYNC_KEYS=["vx","vy","hp","alive","shield","lastPower","lastBasic","stunnedUntil","dotUntil","nextDot","dotDamage","nextRegen","reviveAvailable","revives","damageDone","kills","attacks","hits","dodges","blocks","hitFlash","attackFlash","ultimateReadyAt","ultimateActiveUntil","ultimateDamageMult","ultimateSpeedMult","ultimateLifesteal","ultimateDodge","slowedUntil","slowFactor"];
...
function syncBattle(now){
  ...
  send({type:"battle_state",battle:{time:b.time,over:b.over,randomSeed:state.randomSeed,zone:b.zone,fighters:b.fighters.map(compactFighter),projectiles:b.projectiles,effects:b.effects,texts:b.texts,delayed:b.delayed,ultimatePulses:b.ultimatePulses,ultimateEvent:b.ultimateEvent,ultimateEventId:b.ultimateEventId,actionEvents:b.actionEvents,actionEventId:b.actionEventId,feed:b.feed}});
}
...
function drawBattle(){const b=state.battle;if(!b)return;const W=canvas.width,H=canvas.height;ctx.fillStyle="#0a0a0e";ctx.fillRect(0,0,W,H);ctx.strokeStyle="#1d1d24";ctx.lineWidth=1;for(let x=0;x<W;x+=55){ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,H);ctx.stroke()}for(let y=0;y<H;y+=55){ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(W,y);ctx.stroke()}const z=b.zone;ctx.fillStyle="rgba(255,75,65,.09)";ctx.fillRect(0,0,z.left,H);ctx.fillRect(z.right,0,W-z.right,H);ctx.fillRect(z.left,0,z.right-z.left,z.top);ctx.fillRect(z.left,z.bottom,z.right-z.left,H-z.bottom);ctx.strokeStyle=z.announced?"#ff5c4d":"#44444c";ctx.lineWidth=5;ctx.strokeRect(z.left,z.top,z.right-z.left,z.bottom-z.top);
  for(const shot of b.projectiles)drawProjectile(shot);for(const f of b.fighters)if(f.alive)drawFighter(f);for(const effect of b.effects)drawEffect(effect);for(const text of b.texts){ctx.globalAlpha=Math.max(0,text.life/.8);ctx.fillStyle=text.color;ctx.font="700 15px Rubik";ctx.textAlign="center";ctx.fillText(text.text,text.x,text.y);ctx.globalAlpha=1}updateHud()}
function drawFighter(f){ctx.save();ctx.translate(f.x,f.y);ctx.shadowColor=f.universe.color;ctx.shadowBlur=f.attackFlash>0?28:12;ctx.fillStyle=f.hitFlash>0?"#fff":f.color;ctx.strokeStyle="#050507";ctx.lineWidth=5;ctx.beginPath();ctx.arc(0,0,f.radius,0,Math.PI*2);ctx.fill();ctx.stroke();if(f.shield>0){ctx.strokeStyle="#7de8ff";ctx.lineWidth=4;ctx.globalAlpha=.7;ctx.beginPath();ctx.arc(0,0,f.radius+7,0,Math.PI*2);ctx.stroke();ctx.globalAlpha=1}for(const ex of [-f.radius*.28,f.radius*.28]){ctx.fillStyle="#fff";ctx.strokeStyle="#111";ctx.lineWidth=2;ctx.beginPath();ctx.ellipse(ex,-f.radius*.12,f.radius*.17,f.radius*.25,0,0,Math.PI*2);ctx.fill();ctx.stroke();ctx.fillStyle="#111";ctx.beginPath();ctx.arc(ex+2,-f.radius*.1,Math.max(2,f.radius*.06),0,Math.PI*2);ctx.fill()}const emblem=imageFor(f.universe.icon);if(emblem.complete)ctx.drawImage(emblem,-f.radius*.36,f.radius*.23,f.radius*.72,f.radius*.72);ctx.restore();const w=Math.max(76,ctx.measureText(f.name).width+16),x=f.x-w/2,y=f.y-f.radius-25;ctx.fillStyle="#050507";ctx.fillRect(x-2,y-2,w+4,10);ctx.fillStyle=f.hp/f.maxHp>.5?f.color:f.hp/f.maxHp>.25?"#ffcb42":"#ff5c4d";ctx.fillRect(x,y,w*Math.max(0,f.hp/f.maxHp),6);ctx.fillStyle="#fff";ctx.font="700 11px Rubik";ctx.textAlign="center";ctx.fillText(f.name,f.x,y-5)}
...
function applyHome(){if(state.battle?.raf)cancelAnimationFrame(state.battle.raf);stopBattleMusic();state.battle=null;$("#winnerModal").classList.add("hidden");showScreen("lobbyScreen");renderLobby()}
...
$("#skillHud").addEventListener("click",requestPower);
$("#abilityHud").addEventListener("click",requestUltimate);
canvas.addEventListener("pointerdown",event=>{if(event.button!==0||!$("#battleScreen").classList.contains("active"))return;event.preventDefault();startBattleMusic();const rect=canvas.getBoundingClientRect(),x=(event.clientX-rect.left)*canvas.width/rect.width,y=(event.clientY-rect.top)*canvas.height/rect.height;requestBasicAttack(x,y)});
window.addEventListener("keydown",event=>{if(!["KeyE","KeyF"].includes(event.code)||event.repeat||/INPUT|TEXTAREA|SELECT/.test(event.target.tagName))return;if($("#battleScreen").classList.contains("active")){event.preventDefault();if(event.code==="KeyE")requestPower();else requestUltimate();}});
$("#playAgain").addEventListener("click",()=>{if(isHost())send({type:"home"})});
[$("#onlineName"),$("#onlineColor")].forEach(input=>input.addEventListener("change",()=>{if(room())send({type:"profile",...currentProfile()})}));

$("#onlineName").value=localStorage.getItem("powerRouletteName")||"Shadow";$("#onlineColor").value=localStorage.getItem("powerRouletteColor")||PLAYER_COLORS[Math.floor(Math.random()*PLAYER_COLORS.length)];
