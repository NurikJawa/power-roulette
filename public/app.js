...
function startBattle(seed) {
  ...
  state.battle={fighters,projectiles:[],effects:[],blood:[],texts:[],delayed:[],ultimatePulses:[],ultimateEvent:null,ultimateEventId:0,actionEvents:[],actionEventId:0,feed:[],time:0,serverTime:0,last:performance.now(),accumulator:0,over:false,resultShown:false,lastSync:0,lastHudRender:0,zone:{left:8,right:W-8,top:8,bottom:H-8,start:50,duration:55,maxX:205,maxY:120,announced:false},raf:null};
...
function abilityVisual(power){const value=`${power.id} ${power.name}`.toLowerCase();if(/drill|spiral|бур|спирал/.test(value))return "drill";if(/chainsaw|бензопил/.test(value))return "chainsaw";if(/kagune|rinkaku|ukaku|koukaku|bikaku|кагун|parasite|migi|паразит|миги|щупал/.test(value))return "kagune";if(/notebook|shinigami|тетрад|приговор/.test(value))return "notebook";if(/alchemy|transmut|алхим|трансмута/.test(value))return "alchemy";if(/sun|fire|flame|magma|огн|солн/.test(value)&&/breath|slash|sword|blade|katana|клин|меч|дыхани/.test(value))return "fireBlade";if(/water|вода|водн/.test(value)&&/breath|slash|sword|blade|katana|клин|меч|дыхани/.test(value))return "waterBlade";if(/thunder|lightning|chidori|electric|молн|гром/.test(value))return "lightning";if(/slash|sword|blade|katana|zangetsu|breath|claw|dagger|knife|spear|volundr|клин|меч|дыхани|копь|вёльунд/.test(value))return "blade";if(/fire|flame|magma|atomic|explosion|bomb|огн|взрыв/.test(value))return "explosion";if(/shadow|dark|curse|тен|тьм/.test(value))return "shadow";if(/psychic|telekin|mind|domain|at-field|псих|телекин/.test(value))return "psychic";return power.type}
